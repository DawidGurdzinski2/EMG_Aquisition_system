...
void LCD_DrawImage(uint16_t * ArrayImage,uint8_t ArrayWidth,uint8_t ArrayHeight){

	  //(0,0)      (159,0)
	  //(0,127)		(159,127)
	if(ArrayWidth<=LCD_WIDTH && ArrayHeight<=LCD_HEIGHT ){
		for(uint8_t i=0 ;i<ArrayHeight;i++ ){
			for(uint8_t j =0;j<ArrayWidth;j++){
				LCD_SetCursor (j, i);
				LCD_SetColor ( ArrayImage[i*ArrayWidth+j] , 1 , 1);
			}
		}
	}
}
